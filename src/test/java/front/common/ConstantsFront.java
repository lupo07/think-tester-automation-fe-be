package front.common;

public class ConstantsFront {

	public static final int EXPLICIT_WAIT_ADD_CONTACTS_PAGE = 10;
	public static final int EXPLICIT_WAIT_LOGIN_PAGE = 15;
	public static final int EXPLICIT_WAIT_CONTACTS_PAGE = 10;
	public static final int EXPLICIT_WAIT_CONTACT_DETAILS_PAGE = 10;
	
	public static final String FILE_PATH_CONTACTS_TEST= ".//src//test//resources//";
	public static final String FILE_NAME_CONTACTS_TEST= "ContactsFrontTestData.xlsx";
	
	public static final String LOGIN_PAGE_HEADER = "Contact List App";
	public static final String CONTACTS_PAGE_HEADER = "Contact List";
	public static final String ADD_CONTACTS_PAGE_HEADER = "Add Contact";
	public static final String CONTACTS_DETAILS_PAGE_HEADER = "Contact Details";

}
